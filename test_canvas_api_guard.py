...
class TestInstallerPlan(unittest.TestCase):
    ...
    def test_release_script_points_release_at_main_plus_one_pin_commit(self):
        ...
        shutil.copy(os.path.join(self.ROOT, "canvas_api_guard.py"), os.path.join(repo, "canvas_api_guard.py"))
        os.makedirs(os.path.join(repo, "level2"))
        shutil.copy(os.path.join(self.ROOT, "level2", "canvas_api_operations.py"),
                    os.path.join(repo, "level2", "canvas_api_operations.py"))
        ...
        self.assertIn("Guard: %s\n" % guard.USER_AGENT.split("/")[1], note)
        self.assertIn("Specialized Functions: %s\n" % load_operations().USER_AGENT.split("/")[1], note)
