...
**Known adaptation.** Six of the ten tasks produce prose, which has no red-green cycle. Their red-proof is Step 4: the drift checker fails when a claim recorded in `sources.md` no longer appears on its cited page, which is exactly the failure mode that matters. Task 2 is real code and is written test-first.
