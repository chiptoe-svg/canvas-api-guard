...
| `create-rubric` | converts a flat criteria list into Canvas's indexed shape and reads every criterion back; with `--assignment-id`, the same write attaches it to that assignment with "use this rubric for grading" off, proven by reading the assignment back |
